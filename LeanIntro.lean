...
-- без префикса имя не опознаётся
#check zero
open Nat
#check zero        -- теперь опознаётся


inductive People where
| Mary : People
| John : People
| Bill : People

inductive People₁ where | Mary | John | Bill
